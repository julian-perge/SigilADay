...
{
	private AbilityManager.FullAbility AddProspect()
	{
		AddGoldenNugget();
		// setup ability
		...
		return SigilUtils.CreateAbility<Prospect>(
...
public class GoldenNugget : AbilityBehaviour
{
	...
	public override IEnumerator OnDie(bool wasSacrifice, PlayableCard killer)
	{
		...
		AudioController.Instance.PlaySound3D(
			...
			0.6f,
			0f,
			new AudioParams.Pitch(0.7f),
			null,
			new AudioParams.Randomization()
		);
		yield return new WaitForSeconds(0.25f);
	}
	...
	public static CardInfo GetModdedNugget()
	{
		...
		{
			abilities = new List<Ability> { ability }, nonCopyable = true, nameReplacement = "Golden Nugget"
		};
...
public class Prospect : AbilityBehaviour
{
	...
	public override IEnumerator OnOtherCardDealtDamage(PlayableCard attacker, int amount, PlayableCard target)
	{
		...
		AudioController.Instance.PlaySound3D(
			...
			1f,
			0f,
			new AudioParams.Pitch(AudioParams.Pitch.Variation.Medium)
		);

		yield return new WaitForSeconds(0.25f);
		yield return BoardManager.Instance.CreateCardInSlot(GoldenNugget.GetModdedNugget(), target.Slot);
